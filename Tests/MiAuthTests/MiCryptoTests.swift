...
//    }
    ...
    func testEncryptUART() {
        let expected = Data([0x03, 0x00, 0x00, 0xad, 0xf3, 0x99, 0x08, 0x6b, 0x9e, 0x0b, 0xd0, 0x59, 0x36, 0x6a, 0xd1, 0x0d, 0xfa])

        let key  = Data([0x23, 0x9b, 0x3c, 0x7e, 0x92, 0xdc, 0x6d, 0x6d, 0x2f, 0xa1, 0x74, 0xa2, 0x15, 0xae, 0xdf, 0x2e])
        let iv   = Data([0x01, 0x02])
        let msg  = Data([0x03, 0x20, 0x01, 0x10, 0x0e])
        let rand = Data([0x01, 0x02, 0x03, 0x04])
        let ct = MiCrypto.encryptUart(withKey: key, iv: iv, message: msg, rand: rand)
        XCTAssertEqual(expected.hex(), ct?.hex())
    }
